...
impl ExprLocal for Loc<Expression> {
    ...
    fn handle_call(
        ...
        handle_special_functions! {
            ...
            ["std", "io", "intrinsics", "read_write_inout"] => handle_read_write_inout,
            ["std", "io", "intrinsics", "read_write_items_inout"] => handle_read_write_items_inout,
            ["std", "num", "trunc"] => handle_trunc,
            ...
            |(
                ...
                if ty.backward_size() != BigUint::zero() && ty.size() != BigUint::zero() {
                    if let Some(no_mangle) = no_mangle {
                        return Err(Diagnostic::error(
                            no_mangle,
                            "Parameters with both `inv` and non-`inv` contents cannot be #[no_mangle]",
                        )
                        .primary_label("Not allowed on mixed-direction parameters")
                        .secondary_label(type_spec, "This has both `inv` and non-`inv` components"));
                    }
                }
                ...
                name_source_map
                    .write()
                    .unwrap()
                    .insert_primary(&val_name, NameSource::Name(name_id.clone()));
