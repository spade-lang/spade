...
use spade_hir::{ExecutableItem, ItemList, UnitName, symbol_table::FrozenSymtab};
...
impl MonoState {
    ...
    fn add_mono_traceback(&self, mut diagnostic: Diagnostic, item: &MonoItem) -> Diagnostic {
        let tb = self.get_mono_traceback(item);
        for (loc, message) in tb {
    ...
    pub fn get_mono_traceback(&self, item: &MonoItem) -> Vec<(Loc<()>, String)> {
        let parent = self
            .inner
            .lock()
            .unwrap()
        ...
        if let Some((next_parent, loc)) = parent {
            ...
            let mut result = vec![(loc, format!("{}<{}>", item.source_name, generic_params))];

            result.extend(self.get_mono_traceback(&next_parent));
...
pub struct MirOutput {
    ...
    pub mono_traceback: Vec<(Loc<()>, String)>,
}
    ...
    match original_item {
        Some((ExecutableItem::Unit(u), old_type_state)) => {
            ...
            .map(|mir| MirOutput {
                ...
                type_state: type_state,
                reg_name_map,
                mono_traceback: state.get_mono_traceback(&item)
            });
            out.map(Some)
        }
