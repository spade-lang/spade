...
fn generate_aux_structs(t: Loc<ast::Struct>) -> Result<Vec<Loc<ast::TypeDeclaration>>> {
    if !t.is_port() {
        diag_bail!(t, "The compiler called generate_aux_structs on a non-port struct");
    }
    if let Some(self_) = t.members.self_ {
        diag_bail!(self_, "Found a self on a struct port");
    }
    let (fwd_members, back_members) = t.members.args.iter().map(|(_attrs, name, ty)| {
        match ty {
            spade_ast::TypeSpec::Tuple(_) => ,
            spade_ast::TypeSpec::Array { inner, size } => todo!(),
            spade_ast::TypeSpec::Named(_, _) => todo!(),
            spade_ast::TypeSpec::Unit(_) => todo!(),
            spade_ast::TypeSpec::Inverted(_) => todo!(),
            spade_ast::TypeSpec::Wire(_) => todo!(),
            spade_ast::TypeSpec::Wildcard => todo!(),
        }
    });
}
...
fn forward_type(ty: &Loc<ast::TypeSpec>) -> Option<Loc<ast::TypeSpec>> {
    match &ty.inner {
        spade_ast::TypeSpec::Tuple(members) => {
            let members: Vec<Loc<ast::TypeSpec>> = members.iter().filter_map(forward_type).collect();
            if members.is_empty() {
                None
            } else {
                Some(spade_ast::TypeSpec::Tuple(members).at_loc(&ty))
            }
        },
        spade_ast::TypeSpec::Array { inner, size } => {
            if let Some(inner) = forward_type(inner) {
                Some(spade_ast::TypeSpec::Array { inner: Box::new(inner), size: size.clone() }.at_loc(&ty))
            } else {
                None
            }
        },
        ...
        spade_ast::TypeSpec::Named(name, args) => {
            ...
            Some(spade_ast::TypeSpec::Named(new_name.at_loc(name), args.clone()).at_loc(ty))
        },
        spade_ast::TypeSpec::Unit(_) => None,
        spade_ast::TypeSpec::Inverted(inner) => backward_type(inner),
        spade_ast::TypeSpec::Wire(w) => Some(spade_ast::TypeSpec::Wire(w.clone()).at_loc(&ty)),
        // NOTE: We're leaving this unhandled here. It is an error, but that error will
        // be reported later
        spade_ast::TypeSpec::Wildcard => None,
    }
}

/// Like forward_type, but backward
fn backward_type(ty: &Loc<ast::TypeSpec>) -> Option<Loc<ast::TypeSpec>> {
    match &ty.inner {
        spade_ast::TypeSpec::Tuple(members) => {
            let members: Vec<Loc<ast::TypeSpec>> = members.iter().filter_map(backward_type).collect();
            if members.is_empty() {
                None
            } else {
                Some(spade_ast::TypeSpec::Tuple(members).at_loc(&ty))
        ...
        spade_ast::TypeSpec::Unit(_) => None,
        spade_ast::TypeSpec::Inverted(inner) => forward_type(inner),
        spade_ast::TypeSpec::Wire(_) => None,
        // NOTE: We're leaving this unhandled here. It is an error, but that error will
        // be reported later
        spade_ast::TypeSpec::Wildcard => None,
    }
