...
impl ValueName {
    pub fn unescaped_var_name(&self) -> String {
        match self {
            ValueName::Forward(mir::ValueName::Named(idx, name, _)) => format!(
                "{name}{}",
                if *idx == 0 {
                    format!("")
                } else {
            ...
            ValueName::Backward(mir::ValueName::Named(idx, name, _)) => {
                format!("{name}{}_mut", if *idx == 0 {format!("")} else {format!("_n{idx}")})
            }
